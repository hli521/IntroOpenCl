...
int main(int argc, char **argv){
    ...
    buffer_in = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(float) * 3, NULL, &result);
    if (result != CL_SUCCESS){
        printf("Couldn't initialize input buffer\n");
        return 5;
    }

    buffer_out = clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(float) * 3, NULL, &result);
    if (result != CL_SUCCESS){
    ...
    kernel = clCreateKernel(program, "square_root", NULL);
