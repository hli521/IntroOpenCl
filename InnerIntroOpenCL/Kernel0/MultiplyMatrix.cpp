...
void populateInput(std::vector<std::vector<int>>& matrix, size_t num_rows, size_t num_cols) {
    for (int i = 0; i < num_rows; i++) {
        std::vector<int> row;
        for (int j = 0; j < num_cols; j++) {
            row.push_back(static_cast<int>(std::rand()) % 10);
        }
        matrix.push_back(row);
    }
}

void flatArr(std::vector<std::vector<int>>& matrix, std::vector<int>& out){
    for(int i = 0; i < matrix.size(); i++){
        for(int j = 0; j < matrix.at(i).size(); j++){
            out.push_back(matrix.at(i).at(j));
        }
    }
}

// compute the result of data1 x data2 and store in output as a flattened vector
void computeOutputOnCpu(std::vector<std::vector<int>>& data1, std::vector<std::vector<int>>& data2, std::vector<int>& output) {
    int m = data1.size();
...
bool verifyOutput(std::vector<int>& cpuOutput, std::vector<int>& gpuOutput) {    
...
int main(int argc, char** argv){
    ...
    std::vector<std::vector<int>> data1; // = {{1,2}, {34,22},{213,3}};
    std::vector<std::vector<int>> data2; // = {{21,32,43,12}, {-2,-21,12,2}};
    ...
    result = clEnqueueWriteBuffer(queue, buffer_in1, CL_TRUE, 0, sizeof(int) * m * k, in1.data(), 0, NULL, NULL);
    if (result != CL_SUCCESS){
        printf("Couldn't enqueue write buffer 1\n");
        return 7;
    }

    result = clEnqueueWriteBuffer(queue, buffer_in2, CL_TRUE, 0, sizeof(int) * k * n, in2.data(), 0, NULL, NULL);
    if (result != CL_SUCCESS){
